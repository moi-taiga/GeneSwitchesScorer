...
# binarize atlas data
gs_atlas<- binarize_exp(gs_atlas, fix_cutoff = TRUE, binarize_cutoff = bn_cutoff)
```

## Filter Switching Genes
```{r, eval = FALSE}
sg_allgenes <- filter_switchgenes(gs_atlas, allgenes = TRUE)
sg_allgenes <- filter_switchgenes(gs_atlas, allgenes = TRUE, topnum = 15)
```


### Optional: view the timeline of the filtered switching genes from the Atlas.
```{r, eval = FALSE}
#plot_timeline_ggplot(sg_atlas, timedata = colData(gs_atlas)$Pseudotime, txtsize = 3)
```

## Select evenly distributed switching genes
remember to define the minimum time spacing
```{r, eval = FALSE}
gs_scorer_genes <- select_evenly_distributed_switching_genes(sg_allgenes, min_time_spacing = 5)
```
